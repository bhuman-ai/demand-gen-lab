...
Forward Email is used as the cheap control-probe receiver layer. When `FORWARD_EMAIL_API_TOKEN` and `FORWARD_EMAIL_PROBE_DOMAIN` are set, deliverability probes can create a fresh alias, send the probe from the real sender to that alias, poll the alias over IMAP, and delete the alias after the probe completes. If that Forward Email probe inboxes, the runtime can then queue an optional Gmail confirmation probe using an already-connected Gmail monitor mailbox that has not been used for that sender domain. Gmail is never used before the cheap Forward Email gate passes.
