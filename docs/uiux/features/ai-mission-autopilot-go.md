...
- Autopilot is responsible for deliverability capacity: assign the best existing sender, wait on warmup, and provision a Mailpool domain/inbox when the approved mission policy allows auto provisioning and domain purchase.
...
- 2026-05-16 Deliverability capacity update: Mission start and mission autopilot ticks now run a capacity layer before launch. The layer refreshes sender-launch state, assigns the best existing ready/warming sender when no sender is assigned, treats assigned warming launches as blocked until ready, and can provision a Mailpool sender/domain only when the mission approval policy explicitly allows auto provisioning and auto domain purchase. Provisioning provider failures are recorded as mission decisions/events instead of crashing the mission.
