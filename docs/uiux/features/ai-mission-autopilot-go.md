...
- Autopilot is responsible for deliverability capacity through an AI tool loop: the model chooses whether to inspect, assign a sender, wait on warmup, block for policy, or provision a Mailpool domain/inbox when the approved mission policy allows auto provisioning and domain purchase.
...
- 2026-05-16 Deliverability capacity update: Mission start and mission autopilot ticks now run a capacity layer before launch. The layer refreshes sender-launch state, treats assigned warming launches as blocked until ready, and can provision a Mailpool sender/domain only when the mission approval policy explicitly allows auto provisioning and auto domain purchase. Provisioning provider failures are recorded as mission decisions/events instead of crashing the mission.
- 2026-05-16 AI operator update: Replaced deterministic deliverability capacity selection with a mission deliverability AI operator. The model receives current mission state, sender inventory, launch state, provisioning availability, and guardrails, then chooses one concrete tool: inspect_state, assign_sender, provision_mailpool_sender, wait_for_warmup, or block_for_policy. The code validates policy/spend/readiness but no longer chooses the sender, domain, mailbox local part, or fallback provisioning path for the model.
...
- 2026-05-16 Synced after deliverability capacity update.
- States touched: loading, error, partial
- Code touched: /tmp/lastb2b-mission-operator-live/src/lib/mission-deliverability-capacity.ts, /tmp/lastb2b-mission-operator-live/src/lib/mission-orchestrator.ts, /tmp/lastb2b-mission-operator-live/src/lib/mission-learning.ts, /tmp/lastb2b-mission-operator-live/src/lib/mission-data.ts, /tmp/lastb2b-mission-operator-live/src/lib/mission-types.ts, /tmp/lastb2b-mission-operator-live/src/app/api/brands/[brandId]/missions/route.ts, /tmp/lastb2b-mission-operator-live/src/app/api/brands/[brandId]/missions/[missionId]/start/route.ts, /tmp/lastb2b-mission-operator-live/src/lib/client-api.ts, /tmp/lastb2b-mission-operator-live/docs/uiux/features/ai-mission-autopilot-go.md
