...
## Primary Action
Find recipient emails with Airscale, choose a sender, and send only the currently allowed policy-capped volume. Manual email paste remains a fallback.
...
- Airscale is the default recipient discovery path. Operators paste name/company/domain targets, the shared email-finder batch path resolves only high-confidence sendable work emails, and rejected rows stay visible after launch.
