...
## Request
Change Social Discovery saved-search suggestions so they are brainstormed by an LLM from full brand context and the YouTube comment automation goal, rather than generated from static brand-term heuristics.
## Autonomy Mode
holistic_autopilot
## Target Users
founder/operator
## Optimization Target
better automation search quality and brand-specific relevance
## Hard Constraints
...
- 2026-04-22 Implementation summary: Changed saved-search suggestions from deterministic brand-term heuristics to LLM-brainstormed YouTube search planning. The brainstorm prompt uses full brand context (name, website, tone, product, notes, target markets, ICPs, features, benefits) plus the actual automation goal: find recent YouTube videos from 1k+ subscriber channels where an operator can leave a natural off-the-cuff comment with a casual brand mention. The previous heuristic query builder remains as a fallback when OpenAI is unavailable or the model output is unusable. Social Discovery routes and brand PATCH responses now use the LLM brainstormed suggestions when a brand has no saved searches. UI status copy now says suggested searches come from brand context.
- Files: src/lib/social-discovery.ts, src/lib/llm-router.ts, src/app/api/brands/[brandId]/social-discovery/route.ts, src/app/api/brands/[brandId]/route.ts, src/app/brands/[id]/social-discovery/social-discovery-client.tsx
- Components: SocialDiscoveryClient
- Assumptions used: Suggested searches should be generated by GPT from brand context and the YouTube comment automation goal, while saved searches remain explicit user-approved brand-specific inputs., The old deterministic query builder should stay as fallback so the UI still works without OPENAI_API_KEY.
- 2026-04-22 Implementation summary: Removed the synthetic hardcoded brand-aside fallback from social discovery brand-mention handling. The system no longer rewrites ad-like brand lines into other stock phrases such as 'we see the same at X' variants. Expanded the canned-phrase detector to catch additional variants like 'we noticed that at BRAND' and 'we've noticed that at BRAND'. In the YouTube comment editor, if GPT still returns a canned brand mention after the automatic retry, the draft is withheld and the operator sees an explicit error instead of a patched stock line.
