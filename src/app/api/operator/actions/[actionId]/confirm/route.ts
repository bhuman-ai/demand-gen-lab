...
import { confirmOperatorAction } from "@/lib/operator-runtime";
...
export async function POST(
  ...
  try {
    ...
    const result = await confirmOperatorAction({
      actionId,
    ...
    const status =
      typeof error === "object" && error && "status" in error && typeof error.status === "number"
        ? error.status
        : 500;
    const message = error instanceof Error ? error.message : "Failed to confirm Operator action";
