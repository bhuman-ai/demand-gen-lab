...
async function planActivationWithLlm(input: {
  ...
  const prompt = [
    ...
    "- For sender warmup, Codex should conduct the non-programmatic work itself: use warmup.agent.plan, choose legitimate reply-prone recipients from brand/tool context, write the exact subject/body, then use warmup.agent.enqueue_messages as the generic scheduling hand.",
    "- Do not wait for campaignHopper to invent warmup targets or copy. The agent owns target selection, message writing, and whether to probe/check replies next.",
    "- After agent-authored warmup messages exist, choose deliverability.probe.start when placement evidence is stale or needed before any ramp. Then use reply.check or inbox summaries to judge replies.",
    "- If list/prep is blocked by one sourcing provider, treat the growth tool catalog as a provider menu. Prefer a different lead_source tool such as Airscale when the campaign context gives enough filters, instead of retrying the same failed provider.",
    ...
    "- If warmupAndPrep.due is true and sender reputation/reply acquisition is the likely blocker, choose use_growth_tool with warmup.agent.plan if you need the workbench, or warmup.agent.enqueue_messages if you already have legitimate recipients and exact copy. Use tiny batches.",
