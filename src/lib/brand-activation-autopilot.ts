...
function brandSlug(brand: BrandRecord) {
...
function fallbackDomainCandidates(brand: BrandRecord, preferred = "") {
...
function primaryWebsiteDomain(brand: BrandRecord) {
...
async function buildSnapshots(config: ActivationConfig): Promise<BrandActivationSnapshot[]> {
  ...
  const prioritized = statuses
    .map((status) => {
      ...
      const livePenalty = hasOpenOutboundWork(status) && placementPriority === 0 ? -100 : 0;
      return {
        status,
        score: senderPriority + missionPriority + blockedPriority + placementPriority + livePenalty,
      };
    })
...
async function planActivationWithLlm(input: {
  ...
  const prompt = [
    ...
    "- provision_mailpool_sender: buy/attach a Mailpool sender when the brand has no usable sender.",
    ...
    "- If a brand has no ready/warming/provisioning sender, and domain registration is allowed, choose a sender domain/person so Mailpool can provision it.",
    ...
    "- If a mission is deliverability_blocked because no sending inbox is assigned, and no ready/warming/provisioning sender exists, choose provision_mailpool_sender.",
    "",
  ...
  const plan = normalizeActivationPlan(parsed, input.config.maxActionsPerTick);
