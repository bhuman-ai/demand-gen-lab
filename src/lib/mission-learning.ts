...
import { isOutreachOutboundEnabled } from "@/lib/outreach-account-helpers";
import {
  createMissionEvent,
...
export async function inspectMissionDeliverability(brandId: string): Promise<MissionDeliverabilityState> {
  ...
  const outboundAccounts = activeAccounts.filter((account) => isOutreachOutboundEnabled(account));
  const warmingAccounts = activeAccounts.filter((account) => !isOutreachOutboundEnabled(account));
